 ...
 * Session 数据结构
 */
export interface CustomerGraphSession {
...
export const SESSION_CONFIG = {
  // Session Key 前缀
  keyPrefix: 'customer-graph:session:',
  // Idle timeout (30 分钟)
  ...
  // Cookie 名称
  cookieName: 'customer_graph_session_id',
};
...
export async function createSession(params: {
  ...
  refreshToken?: string;
  expiresIn: number;
  userInfo?: CustomerGraphSession['userInfo'];
}): Promise<CustomerGraphSession> {
  const now = Date.now();
  const session: CustomerGraphSession = {
    id: generateId(32),
 ...
 */
export async function getSession(sessionId: string): Promise<CustomerGraphSession | null> {
  const redis = getRedis();
  ...
  const session: CustomerGraphSession = JSON.parse(data);
  const now = Date.now();
 ...
 * 检查 Token 是否需要刷新
 */
export function shouldRefreshToken(session: CustomerGraphSession): boolean {
  const now = Date.now();
