...
const log = createClientLogger('ClientsErrorBoundary');
...
export default function ClientsError({ error, reset }: ErrorProps) {
  useEffect(() => {
    log.error('客户端页面渲染错误', { message: error.message, digest: error.digest });
  }, [error]);
    ...
    <div className="flex items-center justify-center min-h-[60vh] p-8">
      <Card className="max-w-md w-full border-none shadow-sm ring-1 ring-border/50 rounded-2xl overflow-hidden text-center">
        <CardHeader className="pb-2 pt-8">
          ...
          <CardTitle className="text-xl font-black">应用数据加载失败</CardTitle>
        </CardHeader>
