...
### 4.4 数据范围 (Data Scope) 测试

| 编号 | 测试项 | 步骤 | 预期结果 |
|------|--------|------|----------|
| SCOPE-001 | 全部范围 (ALL) | 超级管理员访问 /api/users | 返回系统中所有用户 |
| SCOPE-002 | 本部门 (DEPT) | 部门管理员访问 /api/users | 仅返回其所属部门的用户 |
| SCOPE-003 | 本部门及下属 (DEPT_AND_SUB) | 经理访问 /api/users | 返回其部门及所有子部门的用户 |
| SCOPE-004 | 个人 (SELF) | 普通用户访问 /api/users | 仅返回其本人的信息 |
| SCOPE-005 | 自定义范围 (CUSTOM) | 指定部门访问 | 返回预设自定义部门的用户 |
