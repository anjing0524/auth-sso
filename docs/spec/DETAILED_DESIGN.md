...
Portal 自身作为 OAuth 2.1 Client，PKCE 在 Gateway（Rust/Pingora）统一生成，proxy.ts 仅检查 `portal_jwt_token` Cookie 存在性，浏览器 JavaScript 零接触敏感凭证。
...
Access Token（1h）过期前 5 分钟（exp-now < 300s），Gateway 服务端静默续签。
...
| Refresh Token | Opaque (DB 存储，SHA-256 哈希存储（token_hash 列）) | 7 天 | Token Rotation + 重用检测（级联撤销）、路径隔离 Cookie |
