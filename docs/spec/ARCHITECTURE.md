...
| **Gateway** | （1）统一 HTTPS 流量入口。（2）OAuth 2.1 Client 层：为所有下游应用统一生成 PKCE/state/nonce → Cookie → 302 /authorize；对配置了 client_secret 的非 OIDC Provider upstream，拦截 callback 代为完成 code→token 交换 + Cookie 下发。（3）提取 `portal_jwt_token` Cookie，通过缓存的 JWKS 验证（ES256、离线）。（4）移除 Cookie，为下游注入 `Authorization: Bearer <JWT>` 头。（5）jti 黑名单检查：验签通过后查询 Redis（`portal:jti_blocklist:{jti}`），Redis 不可用时故障开放（fail-open）。（6）按路径前缀将请求路由到多个上游应用（经 `[[upstreams]]` 路由表，支持 Portal + 子应用） | 绝不执行业务层面的权限检查。绝不连接业务数据库（仅查 Redis jti 黑名单）。OIDC Provider（Portal）自身的 callback 不拦截（由 Portal 自行处理）。 |
...
handle_auth_failure(session)
  → HTML GET 页面导航 + 无 RSC 头 → 透传 Portal（proxy.ts 接管）
