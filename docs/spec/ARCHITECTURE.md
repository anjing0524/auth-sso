...
6. [Portal]      GET /api/auth/callback?code=...
                    ...
                    |- 将令牌写入 HttpOnly Cookie：
                       ...
                       portal_refresh_token（不透明、HttpOnly Secure SameSite=Lax、path=/api/auth/refresh、maxAge=7d）
                    |- 重定向到 /（仪表盘）
```
...
│ · portal_refresh_token（/api/auth/refresh，7d）     │
...
| **刷新令牌（Refresh Token）** | 不透明（数据库中存储） | 7 天 | `portal_refresh_token` HttpOnly Cookie | 静默令牌续期 |
