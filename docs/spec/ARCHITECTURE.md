...
**状态：** 已发布（Released）
**最后更新：** 2026-07-23
**变更：** v5.3 — JWT Claims 最小化（ADR-006）、Gateway callback 统一（ADR-009）、文件引用对齐
...
| **Gateway** | （1）统一 HTTPS 流量入口。（2）OAuth 2.1 Client 层：为所有下游应用统一生成 PKCE/state/nonce → Cookie → 302 /authorize；配置了 `oauth.client_secret` 的所有 upstream（含 Portal 自身），由 Gateway 统一拦截 callback 并完成 code→token 交换 + Cookie 下发。（3）提取 `portal_jwt_token` Cookie，通过缓存的 JWKS 验证（ES256、离线）。（4）移除 Cookie，为下游注入 `Authorization: Bearer <JWT>` 头。（5）jti 黑名单检查：验签通过后查询 Redis（`portal:jti_blocklist:{jti}`），Redis 不可用时故障开放（fail-open）。（6）按路径前缀将请求路由到多个上游应用（经 `[[upstreams]]` 路由表，支持 Portal + 子应用） | 绝不执行业务层面的权限检查。绝不连接业务数据库（仅查 Redis jti 黑名单）。 |
...
| 层 | 允许的依赖 |
|---|---|
| `domain/` | 零外部框架依赖（不依赖 `next/`、`react`、数据库或 Drizzle。可引入 `bcryptjs`、`zod` 等无副作用的 npm 包用于纯函数） |
...
Portal Admin UI 自身的 OAuth Client 职责由 Gateway（Rust/Pingora）统一承担，与第三方 SSO 应用走完全相同的标准链路。

**阶段一：Gateway 生成 PKCE，触发 OAuth 授权流程**

```
1. [浏览器]     GET /dashboard（受保护页面，无 JWT）
2. [Gateway]    检测 HTML GET + 无 JWT + upstream 配置了 oauth
                  |- 生成 PKCE (code_verifier + code_challenge)
                  |- 生成随机 state（CSRF 防护）、nonce（OIDC 防重放）
                  |- 记录回跳路径 return_to
                  |- Set-Cookie: pkce_verifier + oauth_state + oauth_nonce + return_to
                     (Path=/api/auth/callback, HttpOnly, TTL 5min)
                  |- 302 → /api/auth/oauth2/authorize?client_id=portal&code_challenge=...&state=...&nonce=...
```

**阶段二：AS 引导登录 + 授权码签发（authorize → login → authorize）**

```
3. [Portal]      GET /api/auth/oauth2/authorize（首次授权请求）
                    |- 检测无 login_session / portal_jwt_token
                    |- 将 OAuth 参数暂存 Redis: portal:auth_req:{session_id} (TTL 5min)
                    |- 302 → /login?session_id={sid}（只暴露不透明 session_id）

4. [用户]        在 /login 页面提交邮箱 + 密码

5. [Portal]      POST /api/auth/login → 验证凭据 → 签发 login_session JWT (5min)
                    |- 返回 JSON { success: true, redirect: "/api/auth/oauth2/authorize?session_id={sid}" }
                    |- Set-Cookie: login_session（Path=/api/auth/oauth2/authorize）

6. [浏览器]      JS 导航 → GET /api/auth/oauth2/authorize?session_id={sid}
                    |- 从 Redis 恢复 OAuth 参数 → 验 login_session → 一次性消费
                    |- 签发授权码（绑定 code_challenge，5min TTL）
...
5. 验证失败：
   ...
   - **API / RSC / POST 请求** → 返回 `401`，附带 `WWW-Authenticate: Bearer` 头。
...
- **第 10–11 层 · 横切支撑段**：Cookie/Session 生命周期管理与权限上下文缓存，不在每次鉴权请求的关键路径上。注意：第 11 层权限缓存服务于 **token 签发、第 6 层数据范围过滤、`/api/me/permissions`**，以及第 5 层 `checkPermission` 的数据来源——后者从 Redis 读取权限上下文，Redis 不可用时降级为 DB 查询。第 10 层仅在登录/登出/刷新时介入。
...
└──────────────┬──────────────────────────────────┘
               ...
               ▼
┌─────────────────────────────────────────────────┐
│ 第 2 层：proxy.ts（Next.js Proxy）                        │
│ · 公开路径白名单（/login、/oauth2、/.well-known）           │
│ · 静态资源放行（/_next、/favicon、/images、/fonts）         │
│ · Cookie 存在性检查：无 portal_jwt_token → 302 /login       │
│ · PKCE 生成 / OAuth 授权链路已上移到 Gateway 统一完成        │
│ · 不验签（密码学保证由第 1、3 层承担）                       │
└──────────────┬──────────────────────────────────┘
               │
               ▼
...
└──────────────┬──────────────────────────────────┘
               │ userId、claims（sub/jti 身份断言，ADR-006 后无鉴权数据）
               ▼
┌─────────────────────────────────────────────────┐
│ 第 4 层：鉴权守卫（三种形态）                       │
...
│ · 角色码匹配：some / every                          │
│ · 基于 Redis 缓存读取鉴权数据，非 JWT claims        │
│ · 返回 { authorized, userId }                      │
└──────────────┬──────────────────────────────────┘
               │
               ▼
┌─────────────────────────────────────────────────┐
...
authenticate::check(session, ctx, verifier, refresher, is_html_nav)
  → 返回 AuthDecision 枚举（ADR-009）：
    · Pass → 继续代理请求
    · PkceRequired → Gateway 自动生成 PKCE + 302 /authorize（仅 HTML 页面导航）
    · Interrupted → 已向客户端发送 401 响应（API/RSC 请求）
  → 编译器强制穷尽匹配，新增变体时自动报错
```
...
| 防御层 | 组件 | 核心职责 | 失败策略 |
|--------|------|---------|---------|
| **边缘层** | Gateway（Rust/Pingora） | JWT 密码学验签 + kid 匹配 + jti 检查 + Cookie 剥离 + **PKCE 生成** + OAuth 回调拦截（第三方下游应用） | 302 /authorize 或 401 |
| **应用层** | resolveIdentity + withAuth/requirePermission | JWT 自验签 + 权限码匹配 + 数据范围过滤 | 401/403 JSON 响应 |
| **路由层** | proxy.ts | Cookie 存在性检查 + 白名单 + 无 JWT 时 302 /login | 302 /login |
