...
| **H-AUTH-003** | Token 交换 | Portal BFF 通过 Back-Channel 用授权码换取 access_token (ES256 JWT) 和 refresh_token |
| **H-AUTH-004** | JWT Cookie 写入 | Token 交换成功后 `portal_jwt_token` HttpOnly Cookie 被正确设置，包含有效 JWT |
| **H-AUTH-005** | Cookie 安全设置 | `portal_jwt_token` Cookie 正确设置 HttpOnly、Secure (生产环境)、SameSite=Lax 属性 |
| **H-AUTH-010** | State 验证 — 正确 | 正常登录流程中 state 与 Cookie 中存储的 state 匹配，登录成功 |
| **H-AUTH-011** | State 验证 — 错误 | 篡改 URL 中的 state 返回 invalid_state 错误 |
| **H-AUTH-012** | State 过期 | 登录流程暂停超过 10 分钟后 state Cookie 过期，返回 invalid_state 错误 |
| **H-AUTH-013** | PKCE 验证 | Token 请求中包含 code_verifier 参数，IdP 验证 code_challenge (S256) |
| **H-AUTH-014** | Nonce 生成与验证 | 授权 URL 中包含 nonce 参数，回调时校验 ID Token 中的 nonce 匹配 |
...
| ID | 需求点 | 验收标准 |
| :--- | :--- | :--- |
| **H-SESS-001** | JWT Cookie 存在 | 登录成功后浏览器持有 `portal_jwt_token` HttpOnly Cookie 且包含有效 JWT |
| **H-SESS-002** | JWT Claims 完整性 | JWT claims 包含 sub (用户ID)、iss (签发者)、exp (过期时间)、jti (唯一标识)、roles、permissions |
| **H-SESS-003** | JWT 过期时间 | JWT exp 为签发时间 + 1 小时（IdP 配置） |
| **H-SESS-010** | Access Token 过期处理 | Access Token (1h) 过期后 API 返回 401，前端通过 Refresh Token 静默续签 |
| **H-SESS-011** | Refresh Token 过期 | Refresh Token 7 天过期后需重新走完整登录流程 |
| **H-SESS-012** | 前端静默续签 | Access Token 过期前 5 分钟，前端自动调用 /api/auth/refresh 换取新 Token 并更新 Cookie |
| **H-SESS-020** | Token 即将过期自动刷新 | Access Token 剩余有效期 < 5 分钟时前端触发静默刷新 |
| **H-SESS-021** | Token 刷新成功 | 刷新后 `portal_jwt_token` Cookie 被更新为新的 JWT |
| **H-SESS-022** | Token 刷新失败处理 | Refresh Token 失效时清除所有 Cookie 并重定向到登录页 |
| **H-SESS-030** | jti 紧急撤销 | 登出或封禁时 JWT 的 jti 写入 Redis 黑名单 (TTL = Token 剩余有效期)，后续请求返回 401 |
...
| **H-SSO-020** | IdP Session 清除 | Portal 登出时 IdP Session 同步清除 (jti 黑名单 + Refresh Token 撤销) |
| **H-SSO-021** | 登出后受保护页拦截 | 登出后访问受保护页面重定向到登录页 |
| **H-SSO-022** | 登出后重新登录 | 登出后需要重新输入凭证完成 IdP 认证 |
| **SSO-CROSS-APP** | 跨应用单点免登 | Portal 登录后，Demo App 无需输入密码自动建立 Session 登录。 |
| **SSO-DIRECT-ACCESS** | 免登直接访问 | 演示应用识别到 Portal 活动会话后，直接放行访问无需重复认证。 |
| **SSO-LOGOUT-PROPAGATION** | 单点登出联动 | 任何一个应用（如 Portal）登出后，其他关联应用的会话自动失效。 |
