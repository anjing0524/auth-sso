...
flowchart TD
    ...
    subgraph "5. 基础设施层 (Infrastructure Layer)"
        Infra["infrastructure/auth/ | lib/crypto.ts | lib/redis.ts<br/>- 密码哈希、Token 签发等外部适配器"]
    end
...
5.  **基础设施层 (Infrastructure Layer)**：存放密码哈希、Token 签发、外部 API 适配器等**与领域逻辑无关的基础能力**。区别于 `lib/`（全局工具），`infrastructure/` 中的模块可以有外部副作用。
...
│   └── user/                   #   - 聚合根 BC 有界上下文
│       ├── types.ts            #     * Zod Schema 校验与 TS 类型定义
│       └── user.ts             #     * 聚合根纯函数 (状态机、业务逻辑计算)
│
├── infrastructure/             # 3. 基础设施层 (有副作用的外部适配器)
│   └── auth/
│       └── password-service.ts #   - 密码哈希（bcrypt 封装）
│
└── lib/                        # 4. 共享基础设施与公共库
    ├── db.ts                   #   - Drizzle 数据库连接实例 (唯一真相源)
    ├── crypto.ts               #   - 加密、ID 生成等纯工具函数
    ├── redis.ts                #   - Redis 连接实例
    ├── auth/                   #   - 鉴权子模块（拆分自 auth-middleware）
    │   ├── verify-jwt.ts       #     * JWT/Session 身份验证
    │   ├── check-permission.ts #     * 权限编码/角色检查
    │   └── data-scope.ts       #     * 数据范围过滤（含递归 CTE）
    └── auth-middleware.ts      #   - 鉴权统一入口（组合上述子模块）
```
...
import { db } from '@/lib/db';
import { checkPermission, getDataScopeFilter, applyDataScopeFilter } from '@/lib/auth-middleware';
import { headers } from 'next/headers';
    ...
    rules: {
      'boundaries/element-types': [2, {
        ...
        rules: [
          ...
          // 表现层与逻辑胶水层：可以 import domain、lib 和 infrastructure
...
| 源层 → 目标层 | domain | lib (Drizzle/Crypto) | infrastructure | app (Controller/UI) |
| :--- | :---: | :---: | :---: | :---: |
| **domain** | ✅ | ❌ | ❌ | ❌ |
| **lib** | ✅ | ✅ | ❌ | ❌ |
| **infrastructure** | ✅ | ✅ | ✅ | ❌ |
...
| **Server Action** | `actions.ts` | `next/cache`, `domain/`, `lib/` (db/crypto), `infrastructure/` | **≤ 20 行**，Zod `.safeParse()` 校验入参，Drizzle 直调 + 事务，`mapDomainError` 统一错误映射 | `max-lines-per-function` |
| **API 路由** | `route.ts` | `next/server`, `domain/`, `lib/` (db/crypto), `infrastructure/` | REST 写网关，约束同上 | `max-lines-per-function` |
| **领域层核心** | `domain/*/*.ts` | **仅限 `@auth-sso/contracts` + Zod + 纯 TypeScript** | 纯函数计算，业务状态变换，无任何副作用与 DB 依赖。枚举值必须从 contracts 导入，严禁手写 `z.enum(['...'])` 字面量 | `boundaries/element-types` |
...
| **基础设施** | `infrastructure/*/*.ts` | `lib/`, `domain/`, 第三方 SDK（如 `bcryptjs`） | 有副作用的外部适配器（密码哈希、Token 签发等），不可依赖 `app/` | `boundaries/element-types` |
...
| 读模型（data.ts、route.ts）中的 `.toISOString()` | `Date.toISOString()` | 读模型无 Domain 实体参与，直接操作 Drizzle 返回的 Date |
