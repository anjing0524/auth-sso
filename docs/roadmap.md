...
- 2026-07-28: 收敛 Next.js 16 审计读模型的构建期数据库边界：`apps/portal/src/app/audit/data.ts` 不再在模块顶层静态引入 `@/infrastructure/db`，改为请求期惰性装载，消除 `next build` 分析 `/api/audit/export` 等审计路由时提前解析 `DATABASE_URL` 并触发 `Failed to collect page data` 的问题；同时移除 `withPermission` 与审计 Route Handler 中用来止血的 `connection()`，避免继续把 Route Handler 默认动态语义和模块顶层副作用问题混淆，最佳实践沉淀到 `docs/solution/2026-07-28-nextjs-rsc-build-time-db-boundary.md`。
