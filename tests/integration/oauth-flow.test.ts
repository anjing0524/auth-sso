...
async function testAuthorizeEndpoint(): Promise<{ result: TestResult; code?: string; sessionCookies?: Record<string, string> }> {
  ...
  try {
    ...
    // 请求授权
    const authUrl = `${PORTAL_URL}/api/auth/oauth2/authorize?response_type=code&client_id=${OAUTH_CLIENT.clientId}&redirect_uri=${encodeURIComponent(OAUTH_CLIENT.redirectUri)}&scope=openid%20profile%20email&state=${state}&code_challenge=${codeChallenge}&code_challenge_method=S256`;
...
async function testUserInfoEndpoint(accessToken: string): Promise<TestResult> {
  ...
  try {
    const res = await httpRequest(`${PORTAL_URL}/api/auth/oauth2/userinfo`, {
...
async function testUnauthenticatedOidcRedirect(): Promise<TestResult> {
  const start = Date.now();
  const name = '2.1 未登录时 OIDC 授权请求重定向至 /login';
  ...
  try {
    ...
    if (!location) {
      return { name, status: 'FAIL', duration: Date.now() - start, error: 'No Location header in redirect' };
    }

    // 验证是否重定向到本地 Portal /login 页面
    const redirectUrl = new URL(location, PORTAL_URL);
    if (redirectUrl.pathname !== '/login') {
      return { name, status: 'FAIL', duration: Date.now() - start, error: `Expected redirect to /login, got ${redirectUrl.pathname}` };
    }

    // 验证 OIDC 携带的参数是否完整透传至 /login 页面
    const requiredParams = ['client_id', 'redirect_uri', 'state', 'code_challenge', 'code_challenge_method'];
    const missingParams = requiredParams.filter(p => !redirectUrl.searchParams.has(p));

    if (missingParams.length > 0) {
      return { name, status: 'FAIL', duration: Date.now() - start, error: `Missing OIDC params in login redirect: ${missingParams.join(', ')}` };
    }
...
async function runTests() {
  console.log('========================================');
  console.log('Portal IDP Integrated OAuth 2.1 Test');
  console.log('========================================\n');

  results.push(await testHealthCheck());
  results.push(await testSignInEndpoint());

  const { result: authResult, code } = await testAuthorizeEndpoint();
  ...
  results.push(await testUnauthenticatedOidcRedirect());
